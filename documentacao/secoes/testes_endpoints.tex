...
% ---- ENDPOINT 2: POST /api/cadastro ----
\subsection{Endpoint: POST /api/cadastro}

\textbf{Descrição:} Realiza o cadastro de um novo leitor no sistema.

\begin{lstlisting}[style=sqlStyle, caption={Requisição: POST /api/cadastro}]
	POST http://localhost:8080/biblioteca/api/cadastro
	Content-Type: application/x-www-form-urlencoded
...
\begin{lstlisting}[style=sqlStyle, caption={Requisição: GET /api/ativar}]
	GET http://localhost:8080/biblioteca/api/ativar?email=fulano@gmail.com&token=9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d
\end{lstlisting}
...
% ---- ENDPOINT 4: POST /api/recuperar-senha ----
\subsection{Endpoint: POST /api/recuperar-senha}

\textbf{Descrição:} Envia uma senha temporária por e-mail se a conta estiver ativa.

\begin{lstlisting}[style=sqlStyle, caption={Requisição: POST /api/recuperar-senha}]
	POST http://localhost:8080/biblioteca/api/recuperar-senha
	Content-Type: application/x-www-form-urlencoded
